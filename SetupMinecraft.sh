  ...
  if [ "$is_docker" != "yes" ]; then
    ...
    if [ "$is_ubuntu" = "yes" ]; then
      if command -v apt-get &>/dev/null; then
        ...
        if ! command -v unzip &>/dev/null; then DEBIAN_FRONTEND=noninteractive apt-get install unzip -yqq; fi
        if ! command -v route &>/dev/null; then DEBIAN_FRONTEND=noninteractive apt-get install net-tools -yqq; fi
        if ! command -v gawk &>/dev/null; then DEBIAN_FRONTEND=noninteractive apt-get install gawk -yqq; fi
        ...
        UbuntuVer=$(lsb_release -d | cut -f2- | cut -d' ' -f2 | cut -d'.' -f1,2)
          if [[ $(echo "$UbuntuVer >= 22.04" | bc -l) -eq 1 ]]; then
            # Install libssl3 dependency as Bedrock server is linking to both
            CurlVer=$(apt-cache show libssl3 | grep Version | awk 'NR==1{ print $2 }')
            if [[ "$CurlVer" ]]; then DEBIAN_FRONTEND=noninteractive apt-get install libssl3 -yqq; fi
              DEBIAN_FRONTEND=noninteractive apt-get install libc6 -yqq
              DEBIAN_FRONTEND=noninteractive apt-get install libcrypt1 -yqq
          else
            # Install libssl 1.1 if available
            SSLVer=$(apt-cache show libssl1.1 | grep Version | awk 'NR==1{ print $2 }')
            if [[ "$SSLVer" ]]; then DEBIAN_FRONTEND=noninteractive apt-get install libssl1.1 -yqq; fi
            DEBIAN_FRONTEND=noninteractive apt-get install libc6 -yqq
            DEBIAN_FRONTEND=noninteractive apt-get install libcrypt1 -yqq
          fi
        ...
        CPUArch=$(uname -m)
          if [[ "$CPUArch" == *"x86_64"* ]]; then
            echo "No libssl1.1 available in repositories -- attempting manual install"
            curl -sSL -o libssl.deb -k -L https://github.com/TheRemote/Legendary-Bedrock-Container/raw/main/libssl1-1.deb
            dpkg -i libssl.deb
            rm libssl.deb
            SSLVer=$(apt-cache show libssl1.1 | grep Version | awk 'NR==1{ print $2 }')
            if [[ "$SSLVer" ]]; then
...
Update_Server() {
  ...
  curl -sSL -H "Accept-Encoding: identity" -H "Accept-Language: en" -L -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.33 (KHTML, like Gecko) Chrome/90.0.$RandNum.212 Safari/537.33" -o downloads/version.html https://www.minecraft.net/en-us/download/server/bedrock/
